...
describe('records archive page', () => {
  it('renders record filters as DOM text, not interpolated HTML', async () => {
    ...
    globalThis.document = {
      getElementById(id) {
        return id === 'record-filters' ? filters : list;
      },
    ...
    try {
      const { renderFilters } = await import(`../assets/records-page.js?test=${Date.now()}`);
      const category = 'Notes<&>"\'Archive';
